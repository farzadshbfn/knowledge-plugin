...
class KBViewerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _find_kb_loader(self) -> Path | None:
        ...
        # Plugin layout: kb_loader is a sibling skill's script
        sibling = Path(__file__).resolve().parent.parent.parent / "find" / "scripts" / "kb_loader.py"
        if sibling.exists():
            return sibling
        ...
        for entry in self.kb_config.get("kb_roots", []):
            ...
            for candidate in kb_path.rglob("kb_loader.py"):
                if candidate.parent.name == "scripts":
                    return candidate
        # Fallback: search the entire project root
