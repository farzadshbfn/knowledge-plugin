...
As you use your KB, the plugin **tracks access patterns across conversations**. When it detects topics you keep coming back to — the same notes loaded repeatedly, the same questions asked in different contexts — it flags them as **skill candidates**. These are topics mature enough that they'd be better served as a dedicated skill rather than a loose collection of notes.

`/kb-mint` then converts those candidates into standalone skills or packaged plugins for distribution. The pipeline goes: raw notes → structured knowledge → reusable skill → distributable plugin.
